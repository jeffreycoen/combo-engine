# Task 0.0.57-1 — mapgen carved out

One job: move `mapgen` into its own module and leave the depot a one-line front door. Write exactly what is written, run the listed gates, report. You design nothing.

Suggested model: Sonnet 5.

Required reading, in order (confirm at the top of your report):
1. This file, whole.
2. `docs/plans/phase-0.0.57-mapgen.md`, whole.
...
```sh
sha256sum src/depot/mapgen.js   # must print 7a4214747c0bece1e92998b12ec936d8990b373c7aff44b564b0c1c3f153c60f
node scripts/gate.mjs combat | tail -1   # must print: ALL PASS
ls src/modules/mapgen 2>/dev/null || echo absent   # must print: absent
mkdir -p src/modules/mapgen
```

2. Write `src/modules/mapgen/mapgen.js`, exactly (the source file with its import paths rewritten — the one substitution):
...
Then `sha256sum src/modules/mapgen/mapgen.js` — must print `e0f3ff8cb5754896f6310c0cdece0ec17b84b8a3edd0d57b97f344343cf4911c`.

3. Write `src/depot/mapgen.js`, exactly (replacing the whole file):

```js
// mapgen lives in its own module now; this file is the depot's unchanged
// front door — every depot import keeps working.
export * from "../modules/mapgen/mapgen.js";
```

Then `sha256sum src/depot/mapgen.js` — must print `9b095e49a3a4f80dbce35e31d01dca66422c014c7e6c93ac01fdc3d1a1fc65eb`.
...
5. Close the records in this landing: bump `package.json` version to `0.0.57`; in `docs/plans/phase-0.0.57-mapgen.md` replace the status line with `Status: LANDED, commit stamped below, 2026-09-03. Gate: prior gates unmoved, hashes identical.`; in `docs/plans/batch-extractions-3.md` flip `- [ ] 0.0.57 mapgen` to `- [x] 0.0.57 mapgen`; in `README.md` flip `- [ ] mapgen — 0.0.57` to `- [x] mapgen — 0.0.57` — the modules list carries the progress, every landing.
...
```sh
git add src/modules/mapgen/mapgen.js src/depot/mapgen.js package.json README.md docs/plans/phase-0.0.57-mapgen.md docs/plans/task-0.0.57-1-mapgen.md docs/plans/batch-extractions-3.md
git commit -m "phase 0.0.57 — mapgen carved out
...
sed -i "s/commit stamped below/commit \`$H\`/" docs/plans/phase-0.0.57-mapgen.md
git add docs/plans/phase-0.0.57-mapgen.md && git commit -m "phase 0.0.57 record stamped — $H
