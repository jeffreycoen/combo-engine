# Task 0.0.13-1 — the voxel module

One job: write the voxel destruction module and its gate exactly as printed below, register the gate, prove the numbers, close the records. Every file's full content is below; write exactly what is written, run the listed gates, report. You design nothing.

Suggested model: Sonnet 5.

Required reading, in order (confirm at the top of your report):
1. This file, whole.
2. `docs/plans/phase-0.0.13-voxel-module.md`, whole.

Source of the law (reference only — do not edit it): `holdover-greybox-range-r55-claude-opus-5.html` lines 1750–2470 and 2457–2567.
...
ls src/modules/voxel 2>/dev/null || echo absent
```

2. Write `src/modules/voxel/voxel.js`, exactly as printed, ending at the final `;`; the commands after the block set the file's exact ending mechanically, however the writing tool ended the file:
...
```sh
truncate -s 29069 src/modules/voxel/voxel.js   # end exactly at the final character, however the writing tool ended the file
printf '\n' >> src/modules/voxel/voxel.js      # the final line's newline
wc -c src/modules/voxel/voxel.js       # must print 29070
sha256sum src/modules/voxel/voxel.js   # must print 477697cc00b6946dcb6ea8be8c232a42b64c379a564ddd9525ce8e329ab4d1a4
```

3. Write `scripts/voxel-test.mjs`, exactly as printed; the commands after the block set the ending the same way:
...
```sh
truncate -s 6301 scripts/voxel-test.mjs   # end exactly at the final character of the last line
printf '\n' >> scripts/voxel-test.mjs     # the final line's newline
wc -c scripts/voxel-test.mjs       # must print 6302
sha256sum scripts/voxel-test.mjs   # must print 12e2dafa92adab96045cd71cc89df69d31eb477ddc421343de625792150aa730
```

4. In `scripts/gate.mjs`, in the `GATES` table (currently 14 entries ending with `"steering"`), add one line after the `"steering"` entry:
...
5. Run the new gate through the wrapper. The output must be 14 PASS lines, then exactly `voxel-test: 14 PASS / 0 FAIL`, then `voxel-test PASS`, exit 0. Any FAIL stops the task before step 6.
...
7. Close the records in this landing: bump `package.json` version to `0.0.13`; in `docs/plans/phase-0.0.13-voxel-module.md` replace the status line with `Status: LANDED, commit stamped below, 2026-08-28. Gate: 14 PASS / 0 FAIL; prior gates unmoved.`; in `README.md` flip the earned checklist box `- [ ] Voxel destruction: damage only where hit, bored tunnels, support collapse, rubble that stacks` to `- [x]`.
...
```sh
git add src/modules/voxel scripts/voxel-test.mjs scripts/gate.mjs README.md package.json docs/plans
git commit -m "phase 0.0.13 — voxel destruction lands, shaped

Carve and bored tunnels, anchors and collapse, clusters that bake or shatter, rubble that stacks.
voxel-test: 14 PASS / 0 FAIL; fourteen prior gates unmoved.

Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"
git push origin main
H=$(git rev-parse --short HEAD)
sed -i "s/commit stamped below/commit \`$H\`/" docs/plans/phase-0.0.13-voxel-module.md
git add docs/plans && git commit -m "phase 0.0.13 record stamped — $H
...
- Step 5: `voxel-test: 14 PASS / 0 FAIL` then `voxel-test PASS`, exit 0, and an `ok` line in `.superpowers/gates.log`.
...
Read-confirmation first, then one line of outcome, then bullets: the gate's count line and verdict line verbatim, both wc -c lines, both sha256 lines, every prior-gate tail, both commit hashes (landing and stamp), the push results. Every nonconformity its own labeled bullet. Fixture seeds: 9; no seed is special.
