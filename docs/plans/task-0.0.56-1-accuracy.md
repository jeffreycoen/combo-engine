# Task 0.0.56-1 — accuracy carved out

One job: move `accuracy` into its own module and leave the depot a one-line front door. Write exactly what is written, run the listed gates, report. You design nothing.

Suggested model: Sonnet 5.

Required reading, in order (confirm at the top of your report):
1. This file, whole.
2. `docs/plans/phase-0.0.56-accuracy.md`, whole.
...
```sh
sha256sum src/depot/accuracy.js   # must print 05e90d4eea8e712ef3571005c0cc64eb25ea9de866ad2e7161f506d04504d532
node scripts/gate.mjs combat | tail -1   # must print: ALL PASS
ls src/modules/accuracy 2>/dev/null || echo absent   # must print: absent
mkdir -p src/modules/accuracy
```

2. Write `src/modules/accuracy/accuracy.js`, exactly (the source file with its import paths rewritten — the one substitution):
...
import { effRange, fieldReaches } from "../../depot/state.js";
...
Then `sha256sum src/modules/accuracy/accuracy.js` — must print `df5a3f64a2ce16397c4da09684ed7e00d127d7a8170dcebbf100d378f1afd08b`.

3. Write `src/depot/accuracy.js`, exactly (replacing the whole file):

```js
// accuracy lives in its own module now; this file is the depot's unchanged
// front door — every depot import keeps working.
export * from "../modules/accuracy/accuracy.js";
```

Then `sha256sum src/depot/accuracy.js` — must print `0a3815507cb330b286f95bdb3898ca537e7a3d34d9d0fc1152e952653b21f1aa`.
...
5. Close the records in this landing: bump `package.json` version to `0.0.56`; in `docs/plans/phase-0.0.56-accuracy.md` replace the status line with `Status: LANDED, commit stamped below, 2026-09-03. Gate: prior gates unmoved, hashes identical.`; in `docs/plans/batch-extractions-3.md` flip `- [ ] 0.0.56 accuracy` to `- [x] 0.0.56 accuracy`; in `README.md` flip `- [ ] accuracy — 0.0.56` to `- [x] accuracy — 0.0.56` — the modules list carries the progress, every landing.
...
```sh
git add src/modules/accuracy/accuracy.js src/depot/accuracy.js package.json README.md docs/plans/phase-0.0.56-accuracy.md docs/plans/task-0.0.56-1-accuracy.md docs/plans/batch-extractions-3.md
git commit -m "phase 0.0.56 — accuracy carved out
...
sed -i "s/commit stamped below/commit \`$H\`/" docs/plans/phase-0.0.56-accuracy.md
git add docs/plans/phase-0.0.56-accuracy.md && git commit -m "phase 0.0.56 record stamped — $H
