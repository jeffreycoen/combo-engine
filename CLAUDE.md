...
- Never tag a statement with an attribution or a date — no "(owner, ...)", no dated credits, and no workaround that smuggles the same thing in other words. The rule itself is the record. The owner does not issue "rulings".
...
- **NO QUESTION UNTIL THE OWNER IS READY FOR QUESTIONS.** A question is served only when the owner has said he is ready to decide — never in reply to a playtest report, a defect, or a mid-work message. A report gets the diagnosis and the state, full stop; the decision waits until he asks for it. A question with a short option list is never a substitute for stating the whole picture first.
- Question options state their mechanism in the owner's own terms, never buried in jargon or a recommended label; if the owner asked for a thing, that thing is one of the options, verbatim, or the question says plainly why it cannot be.
...
- The phase document's status line carries one of PLANNED, SERVED, APPROVED, DISPATCHED, LANDED, ACCEPTED, RETURNED, moved by the plan-writer at each step. The parts page reads it and nothing else.
...
- **The coldsnap checkout is read-only.** It is read by hash and by line and never written: no plan, no test, no edit, no commit, no push, by the assistant or any agent, for any reason. A fix the engine needs is made here, as a listed difference from the checkout.
...
- Stop after every task: report the landing, then the owner's word decides the next dispatch. A landing includes the deploy — gates green → commit → push, without asking; the owner's live check is the acceptance.
...
- Every dispatch carries a pre-verified reading list and a required read-confirmation opening the agent's report. No trap-notes section: a plan whose steps are copied verbatim carries any warning inside the step it guards.
- Agents verify mechanics, never feel. Look, feel, and sound belong to the owner alone.
- Every landing regenerates the parts table and the parts page and commits them with the landing. The orchestrator republishes the page before the landing report and names it there. A landed phase is ready for acceptance until the owner's live check; it is ACCEPTED only on his word, stamped in the phase document.

## Verification

- Test only what changed; run ONLY the gates the brief lists. No scripted playtesting, ever. The full suite rides CI.
- A tree restored to a state the record already proved green is not tested again. A task that resumes after a stop starts at the first step that changes something.
...
- New screens, panels, and self-contained blocks get their own file, in modules and in games alike. A page's main file and a game's component take only the hookup lines that connect them.
